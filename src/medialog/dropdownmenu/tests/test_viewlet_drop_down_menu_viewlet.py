# -*- coding: utf-8 -*-
from medialog.dropdownmenu.interfaces import IMedialogDropdownmenuLayer
from medialog.dropdownmenu.testing import MEDIALOG_DROPDOWNMENU_FUNCTIONAL_TESTING
from medialog.dropdownmenu.testing import MEDIALOG_DROPDOWNMENU_INTEGRATION_TESTING
...
class ViewletIntegrationTest(unittest.TestCase):
    ...
    def test_drop_down_menu_viewlet_is_registered(self):
        view = BrowserView(self.portal['other-document'], self.request)
        manager_name = 'plone.abovecontenttitle'
        alsoProvides(self.request, IMedialogDropdownmenuLayer)
        ...
        manager.update()
        my_viewlet = [v for v in manager.viewlets if v.__name__ == 'drop-down-menu-viewlet']  # NOQA: E501
        self.assertEqual(len(my_viewlet), 1)

    # XXX would be nice to have this test working:
    # def test_drop_down_menu_viewlet_is_not_available_on_newsitem(self):
...
class ViewletFunctionalTest(unittest.TestCase):
    ...
    layer = MEDIALOG_DROPDOWNMENU_FUNCTIONAL_TESTING
